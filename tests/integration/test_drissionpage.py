...
print("-> Starting movie scraping task: 2 pages with detail page crawling")

chrom = Chromium()

tab = chrom.latest_tab
tab.get('https://www.wangfei.la/vod-show-id-3.html')
...
while current_page <= max_pages:
    ...
    try:
        movie_items = tab.eles(
            "x://div[@class='module-items ']//a[@class='module-poster-item module-item']")
    ...
    for i in range(total_items):
        print(
            f"-> Processing movie {i+1}/{total_items} on page {current_page}")
        ...
        try:
            ...
            try:
                poster_img = item.ele(
                    "x://div[contains(@class,'module-item-pic')]/img")
                poster_url = poster_img.attr(
                    'src') or poster_img.attr('data-src') or ""
            except Exception as e:
                print(f"Warning: Failed to get poster: {e}")

            try:
                status_ele = item.ele(
                    "x://div[contains(@class,'module-item-note')]")
            ...
            try:
                title_ele = tab.ele(
                    "x://div[@class='module-info-heading']/h1/span")
            ...
            try:
                poster_ele = tab.ele(
                    "x://div[@class='module-info-poster']//img")
                if poster_ele:
                    movie_data["poster_image"] = poster_ele.attr(
                        'src') or poster_ele.attr('data-src') or poster_url
            except Exception as e:
                print(f"Warning: Failed to get detail poster: {e}")

            try:
                year_ele = tab.ele(
                    "x://div[@class='module-info-tag']/div[1]//a")
            ...
            try:
                area_ele = tab.ele(
                    "x://div[@class='module-info-tag']/div[2]//a")
            ...
            try:
                genre_ele = tab.ele(
                    "x://div[@class='module-info-tag']/div[3]//a")
            ...
            try:
                alias_ele = tab.ele(
                    "x://div[@class='module-info-items']/div[contains(@class,'module-info-item')][span[text()='别名Alias：']]/div[@class='module-info-item-content']")
            ...
            try:
                update_ele = tab.ele(
                    "x://div[@class='module-info-items']/div[contains(@class,'module-info-item')][span[text()='更新：']]/div[@class='module-info-item-content']")
            ...
            try:
                note_ele = tab.ele(
                    "x://div[@class='module-info-items']/div[contains(@class,'module-info-item')][span[text()='备注：']]/div[@class='module-info-item-content']")
            ...
            try:
                douban_ele = tab.ele(
                    "x://div[@class='module-info-items']/div[contains(@class,'module-info-item')][span[text()='豆瓣：']]/a")
                movie_data["douban_link"] = douban_ele.attr(
                    'href') if douban_ele else ""
            except Exception as e:
                print(f"Warning: Failed to get douban link: {e}")

            try:
                play_eles = tab.eles(
                    "x://div[@class='module-play-list']//a[@class='module-play-list-link']")
                play_links = [ele.attr('href')
                              for ele in play_eles if ele.attr('href')]
                movie_data["play_link"] = "|".join(
                    play_links) if play_links else ""
            except Exception as e:
    ...
    if current_page < max_pages:
        try:
            ...
            next_page_btn = tab.ele(
                "x://*[@id='page']/a[@class='page-link page-next']")
